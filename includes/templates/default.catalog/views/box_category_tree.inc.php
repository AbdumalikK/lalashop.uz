<?php
  $draw_branch = function($category, &$category_path) use (&$draw_branch) {
    ...
    echo '<li class="category-'. $category['id'] . (!empty($category['opened']) ? ' opened' : '') . (!empty($category['active']) ? ' active' : '') .'">' . PHP_EOL
       . '  <a href="'. htmlspecialchars($category['link']) .'">'. functions::draw_fonticon('fa-caret-'. (!empty($category['opened']) ? 'down' : 'right') .' fa-fw') .' '. $category['name'] . ((settings::get('category_tree_product_count') && $category['num_products']) ? ' <small class="pull-right">('. $category['num_products'] .')</small>' : '') .'</a>' . PHP_EOL;
...
<section id="box-category-tree" class="box hidden-xs p-2 bg-white">
